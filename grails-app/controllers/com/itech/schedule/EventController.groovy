...
class EventController {
	...
	def list(Integer max) {
		...
		params.max = Math.max(max ?: Event.count(), 1) //use for datatables
	...
	def range(Integer max){
		params.max = Math.max(max ?: Event.count(), 1) //use for datatables
		def start = params.startDate
		def end = params.endDate
		def query = Event.where{(startDate >= start) && (endDate <= end)}
		def result = query.list(sort:"startDate")
		[eventInstanceList: result]
